...
def main():
    article_url, rss_title, rss_date = latest_post()
    title, date, image_url = parse_article(article_url)

    title = title or rss_title
    date = format_date(date or rss_date)

    if image_url:
        img_data = requests.get(image_url, timeout=30, headers=HEADERS).content
    ...
    draw.text((date_x, 1100), date, fill=BLACK, font=date_font)

    lines = wrap_title(title, title_font, draw, 1000)[:3]
    title_y = 1200
    line_height = 82
